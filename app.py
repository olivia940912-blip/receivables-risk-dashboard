...
st.markdown(
    f"""
    <style>
    hr {{ border: none; border-top: 1px solid {HAIRLINE}; opacity: 1; margin: 1.5rem 0; }}
    div[data-testid="stVerticalBlockBorderWrapper"] {{
        box-shadow: 0 1px 3px rgba(11,11,11,0.06), 0 1px 2px rgba(11,11,11,0.04);
    }}
    [data-testid="stMetric"] {{ padding: 0.75rem 1rem; }}
    </style>
    """,
...
with st.container(border=True):
    st.subheader("[04] 법인별 91일 이상 연체비중")
...
with st.container(border=True):
    st.subheader("[05] 부보기관별 91일 이상 연체비중")
    ...
    verified(
        " · ".join(f"{idx} {r['비율']:.1f}%({int(r['고객수'])}사)" for idx, r in by_insurer.iterrows())
        + " — 전 구간 표본 10건 이상, 원본 CSV 독립 재계산과 일치함"
    )

with st.container(border=True):
    st.subheader("[06] DSO × 여신한도 활용률 산점도")
    ...
    verified(
        f"계산 대상 {len(dso_valid)}사 · DSO 평균 {dso_valid['DSO'].mean():.0f}일/중앙값 {dso_valid['DSO'].median():.0f}일/최댓값 {dso_valid['DSO'].max():.0f}일 · "
        f"무보증초과 {int(m['무보증초과'].sum())}사 — 원본 CSV 독립 재계산과 일치함"
    )
